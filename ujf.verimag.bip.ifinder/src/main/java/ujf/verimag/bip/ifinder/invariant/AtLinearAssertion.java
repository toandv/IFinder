...
/*
 * linear assertions involving at- variables
 *
 */

public class AtLinearAssertion extends BasicAssertion {

    public enum Operator {LE, EQ, GE};
    
    // members
    protected HashMap<AtVariable,Integer> m_term;
    protected Operator m_operator;
    protected int m_bound;
    
    public AtLinearAssertion(Operator operator) {
	m_term = new HashMap<AtVariable,Integer>();
	m_operator = operator;
	m_bound = 0;
    }
    
    public void add(AtVariable variable, Integer coefficient) {
	...
	AtVariable variables[] = m_term.keySet().toArray( new AtVariable[0] );
