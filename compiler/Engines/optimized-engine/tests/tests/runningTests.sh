...
tests_clock_speed() {
  ...
  for index in {0..98}
  do
    let "time=$index*10+10"
    let "success+=$(grep "choose.*A.p1.*global time $time$ms" -c $OUTPUT_DIR/trace)"
  done

  let "success+=$(grep 'choose.*A.p1.*global time 1s$' -c $OUTPUT_DIR/trace)"

  for index in {100..199}
  do
    let "time=$index*10-990"
    let "success+=$(grep "choose.*A.p1.*global time 1s$time$ms" -c $OUTPUT_DIR/trace)"
  done
  
  for index in {0..198}
  do
    let "success+=$(grep "state.*#$index .*1.*internal.*port" -c $OUTPUT_DIR/trace)"
  done
  
  let "success+=$(grep 'choose.*A.p1.*global time 2s$' -c $OUTPUT_DIR/trace)"

  grep -q 'state #200.*deadlock' $OUTPUT_DIR/trace &&
  [ $success -eq "399" ]
...
tests_test_resume() {
  ...
  grep -q 'choose.*A.p2'                  $OUTPUT_DIR/trace &&
...
tests_test_resume3() {
  ...
  grep -q 'choose.*C1.*A.p.*B.p'                 $OUTPUT_DIR/trace &&
...
tests_test_resume4() {
  ...
  grep -q 'state #0.*:.*1 internal port'  $OUTPUT_DIR/trace &&
  grep -q 'choose.*A.p'                   $OUTPUT_DIR/trace &&
  grep -q 'state #1.*:.*deadlock'         $OUTPUT_DIR/trace &&
  grep -q 'state #0.*:.*1 internal port'  $OUTPUT_DIR/trace-real-time &&
...
tests_exernal() {
  ...
  compute_output_dir
  compile_to_cpp "$TEST" "$RUNNING_TESTS" 'Compound()' "--gencpp-cc-I $RUNNING_TESTS/external --gencpp-cc-extra-opt -pthread --gencpp-ld-extra-opt -pthread" && compile_and_link_with_engine "$TEST" || fail "Compiler returned an error"
...
test_priority_rules_guard_external() {
  ...
  NB=$(grep -e 'state.*2.*interactions' -c $OUTPUT_DIR/trace)
